        ...
        while (not xbmc.abortRequested):
            if xbmcaddon.Addon().getSetting("library_import") == 'true':
                try:
                    ...
                    hoursList = [1, 2, 5, 10, 15, 24]
                    interval = int(xbmcaddon.Addon().getSetting("library_interval"))
                    ...
                    if not (xbmc.Player().isPlaying() or xbmc.getCondVisibility('Library.IsScanningVideo')):
                        xbmc.executebuiltin('RunPlugin(plugin://plugin.video.GOmovies/?action=library_service)')
                        xbmcaddon.Addon().setSetting('library_run', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"))
                except:
                    pass
            xbmc.sleep(1000)
