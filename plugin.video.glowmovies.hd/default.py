...
class root:
    def get(self):
        rootList = []
        rootList.append({'name': 30501, 'image': 'All.png', 'action': 'movies_all'})
        rootList.append({'name': 30502, 'image': 'Added.png', 'action': 'movies_added'})
        rootList.append({'name': 30503, 'image': 'Genres.png', 'action': 'genres_movies'})
        rootList.append({'name': 30504, 'image': 'Years.png', 'action': 'years_movies'})
...
class genres:
    ...
    def glow_list(self):
        try:
            result = getUrl(link().glow_genres).result
            result = result.decode('iso-8859-1').encode('utf-8')
            genres = common.parseDOM(result, "td", attrs = { "class": "topic_content" })
        except:
            return
...
class years:
    ...
    def glow_list(self):
        try:
            result = getUrl(link().glow_years).result
            result = result.decode('iso-8859-1').encode('utf-8')
            years = common.parseDOM(result, "td", attrs = { "class": "topic_content" })
        except:
            return
...
class movies:
    ...
    def get(self, url):
        if 'search.php' in url:
            #self.list = self.glow_list(url)
            self.list = cache(self.glow_list, url)
        else:
            #self.list = self.glow_list2(url)
            self.list = cache(self.glow_list2, url)
    ...
    def glow_list(self, url):
        try:
            result = getUrl(url).result
            result = result.decode('iso-8859-1').encode('utf-8')

            try:
                p = re.findall('page=(\d+)', url, re.I)[0]
                r = common.parseDOM(result, "form", ret="action", attrs = { "class": "pagination.+?" })[0]
                r = '%s/%s&page=%s' % (link().glow_forum, r, p)
                r = common.replaceHTMLCodes(r)
                result = getUrl(r).result
                result = result.decode('iso-8859-1').encode('utf-8')
            except:
                pass

            movies = common.parseDOM(result, "div", attrs = { "class": "threadinfo thread" })
        except:
            return

        try:
            next = common.parseDOM(result, "a", ret="href", attrs = { "rel": "next" })[-1]
            next = common.replaceHTMLCodes(next)
            next = re.findall('.+?(&page=\d+)', next, re.I)[0]
            next = '%s%s' % (url.rsplit('&page=', 1)[0], next)
        ...
        for movie in movies:
            try:
                name = common.parseDOM(movie, "a", attrs = { "class": "title" })[0]
                name = re.findall('(.+?[(]\d{4}[)])', name, re.I)[0]
                ...
                try:
                    ...
                    imdb = re.findall('tt(\d{4}\d+)', imdb, re.I)[0]
                ...
                try:
                    genre = common.parseDOM(movie, "a", ret="title", attrs = { "class": "title" })[0]
                    genre = re.findall('Genre:(.+?)  ', genre, re.I)[0]
                ...
                self.list.append({'name': name, 'url': url, 'image': image, 'title': title, 'year': year, 'imdb': imdb, 'genre': genre, 'plot': '', 'next': next})
            except:
                pass

        return self.list

    def glow_list2(self, url):
        try:
            result = getUrl(url).result
            result = result.decode('iso-8859-1').encode('utf-8')
            movies = common.parseDOM(result, "td", attrs = { "class": "topic_content" })
        except:
            return

        try:
            next = common.parseDOM(result, "div", attrs = { "class": "mainpagination" })[0]
            next = re.findall('href="(.+?)">><', next, re.I)[0]
            next = '%s%s' % (link().glow_base, next)
            next = common.replaceHTMLCodes(next)
            next = next.encode('utf-8')
        except:
            next = ''
        ...
        for movie in movies:
            try:
                ...
                try:
                    genre = common.parseDOM(movie, "pre", attrs = { "class": "imgDescription" })[0]
                    genre = genre.split('\n')[0].split('>', 1)[-1].strip()
                    genre = genre.replace(' | ', ' / ').strip()
                    genre = common.replaceHTMLCodes(genre)
                    genre = genre.encode('utf-8')
...
class resolver:
    ...
    def glow(self, url, name):
        ...
        try:
            ...
            url = getUrl(url, referer=link().glow_base).result
            url = common.parseDOM(url, "iframe", ret="src")[0]
            url = getUrl(url, referer=link().glow_base).result

            u = re.compile('"((?:http|https)://redirector.googlevideo.com/.+?)"').findall(url)
            u += re.compile('"((?:http|https)://docs.google.com/.+?)"').findall(url)
            url = u[-1]
